  ...
  PetscErrorCode solve()
    {
      ...
      ierr = DMDACreate2d(PETSC_COMM_WORLD, DM_BOUNDARY_NONE, DM_BOUNDARY_NONE,DMDA_STENCIL_STAR,3,3,PETSC_DECIDE,PETSC_DECIDE,1,1,0,0,&da);
      ...
      user.rho    = 1.0;
      ierr        = PetscOptionsReal("-rho", "The conductivity", "ex29.c", user.rho, &user.rho, NULL);
      user.nu     = 0.1;
      ierr        = PetscOptionsReal("-nu", "The width of the Gaussian source", "ex29.c", user.nu, &user.nu, NULL);
      bc          = (PetscInt)DIRICHLET;
      ierr        = PetscOptionsEList("-bc_type","Type of boundary condition","ex29.c",bcTypes,2,bcTypes[0],&bc,NULL);
...
{
  Container * c = new Container(argc, argv);
  c->solve();
  delete c;
  return 0;
}
